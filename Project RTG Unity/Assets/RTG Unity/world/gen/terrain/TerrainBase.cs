    ...
    public abstract class TerrainBase
{
    ...
    public static readonly float minimumOceanFloor = 30.01f; // The lowest Y coord an ocean floor is allowed to be.
    public static readonly float minimumDuneHeight = 21f; // The strength factor to which the dune height config option is added.
    protected readonly float minOceanFloor; // The lowest Y coord an ocean floor is allowed to be.
    protected readonly float minDuneHeight; // The strength factor to which the dune height config option is added.
    protected readonly float groundNoiseAmplitudeHills;
    protected readonly float groundVariation;
    protected readonly float rollingHillsMaxHeight;
    protected float _base; // added as most terrains have this;
    protected float _groundNoise;
    protected RTGConfig rtgConfig = RTGAPI.config();
    protected static RTGConfig configRTG = RTGAPI.config();

    public TerrainBase() : this(68f)
    {
        // default to marginally above sea level;
    }

    public TerrainBase(float _base)
    {
    ...
    public static float borderAdjusted(float effect, float border, float allAbove, float noneBelow)
    {
    ...
    public static float stretched(float toStretch)
    {
    ...
    public static float hills(float x, float y, float hillStrength, OpenSimplexNoise simplex, float river)
    {
        ...
        float sm = (float)simplex.octaves[2].Evaluate(x / 55, y / 55);// there are artifacts if this is close to a multiple of 16
    ...
    public static float groundNoise(int x, int y, float amplitude, OpenSimplexNoise simplex)
    {
        ...
        h += blendedHillHeight((float)simplex.octaves[1].Evaluate((float)x / 23f, (float)y / 23f), 0.2f) * amplitude / 2f;
        h += blendedHillHeight((float)simplex.octaves[2].Evaluate((float)x / 11f, (float)y / 11f), 0.2f) * amplitude / 4f;
        return h;
    }
    ...
    public static float groundNoise(float x, float y, float amplitude, OpenSimplexNoise simplex)
    {
        ...
        h += blendedHillHeight((float)simplex.octaves[1].Evaluate(x / 23f, y / 23f), 0.2f) * amplitude / 2f;
        h += blendedHillHeight((float)simplex.octaves[2].Evaluate(x / 11f, y / 11f), 0.2f) * amplitude / 4f;
        return h;
    }
    ...
    public static float terrainBryce(int x, int y, OpenSimplexNoise simplex, float river, float height, float border)
    {
        ...
        sn += simplex.noise2(x, y) * 0.2f + 0.2f;
    ...
    public static float terrainCanyon(int x, int y, OpenSimplexNoise simplex, float river, float[] height, float border, float strength, int heightLength, bool booRiver)
    {
    ...
    public static float terrainFlatLakes(int x, int y, OpenSimplexNoise simplex, float river, float hMax, float baseHeight)
    {
        ...
        float ruggedNoise = (float)simplex.octaves[1].Evaluate(x / VariableRuggednessEffect.STANDARD_RUGGEDNESS_WAVELENGTH, y / VariableRuggednessEffect.STANDARD_RUGGEDNESS_WAVELENGTH);
    ...
    public static float terrainGrasslandHills(int x, int y, OpenSimplexNoise simplex, CellNoise cell, float river, float vWidth, float vHeight, float hWidth, float hHeight, float bHeight)
    {
        ...
        float m = (float)simplex.octaves[1].Evaluate(x / hWidth, y / hWidth);
    ...
    public static float terrainGrasslandMountains(int x, int y, OpenSimplexNoise simplex, CellNoise cell, float river, float hFactor, float mFactor, float baseHeight)
    {
        ...
        float c = (float)simplex.octaves[4].Evaluate(x / 30f, y / 30f, 1D) * (m * 0.30f);
    ...
    public static float terrainHighland(float x, float y, OpenSimplexNoise simplex, CellNoise cell, float river, float start, float width, float height, float baseAdjust)
    {
        ...
        if (h > 0f)
        {
            ...
            h += (float)simplex.octaves[4].Evaluate(x / 70D, y / 70D, 1D) * st;// 0 to 155
            h = h * river;
        }
    ...
    public static float terrainLonelyMountain(int x, int y, OpenSimplexNoise simplex, CellNoise cell, float river, float strength, float width, float terrainHeight)
    {
        ...
        float st = m * 0.7f;
        st = st > 20f ? 20f : st;
        float c = (float)simplex.octaves[4].Evaluate(x / 30f, y / 30f, 1D) * (5f + st);
    ...
    public static float terrainMountainRiver(int x, int y, OpenSimplexNoise simplex, CellNoise cell, float river, float hPitch, float baseHeight)
    {
        ...
        if (h > 10f)
        {
            ...
            if (h > 35f)
            {
                ...
                h += (float)simplex.octaves[4].Evaluate(x / 25D, y / 25D, 1D) * d2;
            }
        }
    ...
    public static float terrainMountainSpikes(int x, int y, OpenSimplexNoise simplex, CellNoise cell, float river)
    {
        ...
        float h = (float)simplex.octaves[4].Evaluate(x / 200D, y / 200D, 1D) * b * river;
        h *= h * 1.5f;
        h = h > 155f ? 155f : h;
    ...
    public static float terrainOceanCanyon(int x, int y, OpenSimplexNoise simplex, float river, float[] height, float border, float strength, int heightLength, bool booRiver)
    {
        //float b = simplex.noise2(x / cWidth, y / cWidth) * cHeigth * river;
        //b *= b / cStrength;
        river *= 1.3f;
    ...
    public static float terrainPlateau(float x, float y, OpenSimplexNoise simplex, float river, float[] height, float border, float strength, int heightLength, float selectorWaveLength, bool isM)
    {
    ...
    public static float terrainRollingHills(int x, int y, OpenSimplexNoise simplex, float river, float hillStrength, float addedHeight, float _groundNoise, float groundNoiseAmplitudeHills, float lift)
    {

        _groundNoise = groundNoise(x, y, groundNoiseAmplitudeHills, simplex);

        float m = hills(x, y, hillStrength, simplex, river);

        float floNoise = addedHeight + _groundNoise + m;

        return riverized(floNoise + lift, river);
    }

    public static float terrainRollingHills(int x, int y, OpenSimplexNoise simplex, float river, float hillStrength, float _groundNoise, float groundNoiseAmplitudeHills, float baseHeight)
    {
